...
dbapi.loadUserDevicesFilteredByGroupMembership = function(email) {
  ...
  //var userGroups = db.run(r.table('userGroups')
  //  .filter(r.row['userEmails'].indexOf(email) !== -1))

  return db.run(r.table('userGroups').filter(function(ugroup) {
    return true
    //return ugroup('fingerprint').ne(fingerprint)
  })).then(function(cursor) {
    return Promise.promisify(cursor.toArray, cursor)()
      .then(function(userGroups) {
        ...
        db.run(r.table('deviceGroups')
          .filter(function(dgroup) {
            return true
            //dgroup.userGroupTitles.some(title => userGroupTitles.includes(title))
          })).then(function(cursor) {
          return Promise.promisify(cursor.toArray, cursor)()
