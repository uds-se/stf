...
var TestExecutor = require('../executor/executor.js').TestExecutor
var CleanUpExecutor = require('../executor/executor.js').DockerCleanUpExecutor
...
module.exports = function(options) {
  ...
  const CLEAN_UP_TIME_INTERVAL_IN_DAYS = 1

  // TODO not really the proper place for this kind of start-up logic
  // Create an admin user group if non existent
  dbapi.createAdminGroup()

  // Setup a periodic cronjob
  const job = new CronJob(`0 0 */${CLEAN_UP_TIME_INTERVAL_IN_DAYS} * * *`, function() {
    const executor = new CleanUpExecutor(log, options.saveDir)
    executor.cleanup().then(function() {
  ...
  io.on('connection', function(socket) {
    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        //--------------- Device Groups
        .on('group.device.group.add', function(data) {
          return dbapi.lookupDeviceGroupByTitle(data.title)
            ...
            .then(function(groups) {
              if (groups.length) {
                ...
                return dbapi.insertDeviceGroup(data).then(function() {
                  socket.emit('group.device.group.added', {
                    title: data.title
                    , deviceSerials: []
                  })
                })
          ...
          .catch(dbapi.DuplicateSecondaryIndexError, function(err) {
            socket.emit('group.device.group.error', {
              message: 'Someone already added this device group'
            })
          })
          .catch(Error, function(err) {
            socket.emit('group.device.group.error', {
        ...
        .on('group.device.group.remove', function(data) {
          return dbapi.removeDeviceGroup(data)
          .then(function() {
            socket.emit('group.device.group.removed', data)
          })
        })
        .on('group.device.group.device.add', function(group, device) {
          if (group.deviceSerials.indexOf(device.serial) !== -1) {
            socket.emit('group.device.group.device.error', {
              message: 'Someone already added this device to the group'
            })
          }
          else {
            group.deviceSerials.push(device.serial)

            return dbapi.insertDeviceIntoGroup(group)
              .then(function() {
                socket.emit('group.device.group.device.added', group)
              })
              .catch(dbapi.DuplicateSecondaryIndexError, function(err) {
                socket.emit('group.device.group.device.error', {
                  message: 'Someone already added this device to the group'
        ...
        .on('group.device.group.device.remove', function(group, device) {
          return dbapi.removeDeviceFromGroup(group, device)
            .then(function() {
              socket.emit('group.device.group.device.removed', group)
            })
        })
        .on('group.device.group.ugroup.add', function(group, ugroup) {
          if (group.userGroupTitles.indexOf(ugroup.title) !== -1) {
            socket.emit('group.device.group.ugroup.error', {
              message: 'Someone already added this user group to the device group'
            })
          }
          else {
            group.userGroupTitles.push(ugroup.title)

            return dbapi.associateUserGroupWithDeviceGroup(group)
              .then(function() {
                socket.emit('group.device.group.ugroup.added', group)
              })
              .catch(dbapi.DuplicateSecondaryIndexError, function(err) {
                socket.emit('group.device.group.device.error', {
                  message: 'Someone already added this user group to the device group'
        ...
        .on('group.device.group.ugroup.remove', function(group, userGroupTitle) {
          return dbapi.removeAssociatedUserGroupFromGroup(group, userGroupTitle)
            .then(function() {
              socket.emit('group.device.group.ugroup.removed', group)
            })
        })
        //--------------- User Groups
        .on('group.user.group.add', function(data) {
          return dbapi.lookupUserGroupByTitle(data.title)
            .then(function(groups) {
              return groups.toArray()
            })
            .then(function(groups) {
              if (groups.length) {
                ...
                return dbapi.insertUserGroup(data).then(function() {
                  socket.emit('group.user.group.added', {
                    title: data.title
                    , userEmails: []
                  })
                })
            ...
            .catch(dbapi.DuplicateSecondaryIndexError, function(err) {
              socket.emit('group.user.group.error', {
                message: 'Someone already added this user group'
              })
            })
            .catch(Error, function(err) {
              socket.emit('group.user.group.error', {
        ...
        .on('group.user.group.remove', function(data) {
          return dbapi.removeUserGroup(data)
            .then(function() {
              socket.emit('group.user.group.removed', data)
            })
        })
        .on('group.user.group.user.add', function(group, user) {
          if (group.userEmails.indexOf(user.email) !== -1) {
            socket.emit('group.user.group.user.error', {
              message: 'Someone already added this user to the group'
            })
          } else {
            group.userEmails.push(user.email)

            return dbapi.insertUserIntoGroup(group)
              .then(function() {
                socket.emit('group.user.group.user.added', group)
              })
              .catch(dbapi.DuplicateSecondaryIndexError, function(err) {
                socket.emit('group.user.group.user.error', {
                  message: 'Someone already added this user to the group'
        ...
        .on('group.user.group.user.remove', function(group, user) {
          return dbapi.removeUserFromGroup(group, user)
            .then(function() {
              socket.emit('group.user.group.user.removed', group)
            })
        })
        // Touch events
        ...
        .on('group.invite', function(channel, responseChannel, data) {
          joinChannel(responseChannel)
          // Check backend side if user is allowed to use an additional device
          return dbapi.isUserAllowedToUseAdditionalDevice(user.email)
            .then(function(isAllowed) {
              if (!isAllowed) {
                throw new Error('User is not allowed to access this device')
              }
              else {
                push.send([
                  channel
        ...
        //--------------- Testing Tools
        .on('testing.tool.add', function(data) {
          return dbapi.lookupUserTestingToolbyTitle(user.email, data.title)
            .then(function(tools) {
            ...
            .then(function(tools) {
              if (tools.length) {
                ...
                return dbapi.insertUserTestingTool(user.email, data).then(function() {
                  socket.emit('testing.tool.added', {
                    title: data.title
                    , gitRepository: data.gitRepository
                    , gitCommit: data.gitCommit
                    , setupParameters: data.setupParameters
                    , dockerRepository: data.dockerRepository
                  })
                })
              }
            })
            .catch(dbapi.DuplicateSecondaryIndexError, function(err) {
              socket.emit('testing.tool.error', {
                message: 'A testing tool with this name already exists'
              })
            })
            .catch(Error, function(err) {
              socket.emit('testing.tool.error', {
        ...
        //--------------- Commands
        .on('command.testing.tools.execute', function(tests, storageId) {
          const executor = new TestExecutor(socket, log, user.email, options.saveDir)
