  ...
  io.on('connection', function(socket) {
    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('command.execute.droidmate.jar', function(params, outputDir) {
          let droidmatePath = '/opt/droidmate/'
          let cmd = `cd ${droidmatePath}; java -jar ${droidmatePath}droidmate.jar ${params}`
          log.info('Executed command:', cmd)
          exec(cmd, function(error, stdout, stderr) {
            if (stdout !== '') {
              socket.emit('command.reply', stdout)
            }
            if (stderr !== '') {
            ...
            // Zip reports and logs
            var zipPath = path.join(path.resolve(outputDir, '..'), '/output.zip')
        ...
        // Deprecated
        .on('command.execute.droidmate.gradle', function(params) {
          var droidmatePath = '/opt/droidmate/dev/droidmate/'
          var argsFile = droidmatePath + 'args.txt'
          fs.writeFileSync(argsFile, params, 'utf8')
          log.info('Used params: ', params)

          let cmd = `cd ${droidmatePath}; ${droidmatePath}gradlew :projects:command:run`
          log.info('Executed command:', cmd)
          exec(cmd, function(error, stdout, stderr) {
            if (stdout !== '') {
              socket.emit('command.reply', stdout)
