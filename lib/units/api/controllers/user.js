...
*/
function getNumberOfUsagesByGroup(dgroup, serialDeviceMap, tmpSerials) {
  let resolver = Promise.defer()
  let numUsages = 0
  let counter = 0
...
function getNumberOfUsagesByGroups(dgroups, serialDeviceMap) {
  let resolver = Promise.defer()
  let numUsages = 0
  ...
  dgroups.forEach(function(dgroup) {
    getNumberOfUsagesByGroup(dgroup, serialDeviceMap, tmpSerials).then(function(usages) {
      counter++
      numUsages += usages
...
function getAllSerials(userGroups, deviceGroups) {
  let resolver = Promise.defer()
  ...
  userGroups.forEach(function(ugroup) {
    ...
    if (dgroups !== undefined) {
      getNumberOfUsagesByGroups(dgroups, serialDeviceMap).then(function(data) {
        counter++
...
function getUserDevicesFilteredByGroupMembership(req, res) {
  ...
  dbapi.loadUserDevicesFilteredByGroupMembership(req.user.email)
    .then(function(groups) {
      ...
      getAllSerials(userGroups, deviceGroups).then(function(serialSet) {
