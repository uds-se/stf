...
class TestExecutor {
  ...
  static checkIfDockerDaemonIsRunning() {
    const cmd = `docker stats --no-stream`
    log.info(cmd)
    ...
    try {
      ...
      assert(stdout.includes('Cannot connect to the Docker daemon'))
      return false
    }
  }
...
module.exports = function(_options) {
  ...
  if (!TestExecutor.checkIfDockerDaemonIsRunning()) {
    throw Error("Docker daemon is not running.")
  }
